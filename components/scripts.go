...
function drawSkeleton(ctx, joints, width, height, options = {}) {
  ...
  function getCanvasCoords(jointName) {
    // Для OpenPose у нас 'spine' вместо 'chest'
    if (jointName === 'chest') jointName = 'spine';
    ...
    if (options.videoRect) {
       ...
       if (options.isOpenPose) {
           // OpenPose возвращает абсолютные пиксели
           return {
              x: nx * options.videoRect.scale + options.videoRect.dx,
              y: ny * options.videoRect.scale + options.videoRect.dy
           };
       }
...
function drawOpenPoseOverlay(canvasIncorrect, jump, currentFrameIndex) {
  if (!canvasIncorrect || !jump) return;
  ...
  // Отрисовка самого видео на заднем фоне (удалено)
  let vRect = null;
  ...
  // Берем кадр из OpenPose таймлайна
  let currentSkeleton = null;
  if (window.openPoseTimeline && window.openPoseTimeline.length > 0) {
      // Подбираем индекс кадра на основе текущего времени видео
      const progress = videoPlayer.currentTime / videoPlayer.duration;
      let frameIdx = Math.floor(progress * window.openPoseTimeline.length);
      if (frameIdx >= window.openPoseTimeline.length) frameIdx = window.openPoseTimeline.length - 1;
      currentSkeleton = window.openPoseTimeline[frameIdx];
  }
  ...
  let legacyJoints = currentSkeleton;
  if (currentSkeleton.joints) {
      legacyJoints = {
          head: currentSkeleton.joints.Nose || {x:0,y:0},
          spine: currentSkeleton.joints.Neck || currentSkeleton.joints.MidHip || {x:0,y:0},
          shoulderL: currentSkeleton.joints.LShoulder || {x:0,y:0},
          shoulderR: currentSkeleton.joints.RShoulder || {x:0,y:0},
          elbowL: currentSkeleton.joints.LElbow || {x:0,y:0},
          elbowR: currentSkeleton.joints.RElbow || {x:0,y:0},
          wristL: currentSkeleton.joints.LWrist || {x:0,y:0},
          wristR: currentSkeleton.joints.RWrist || {x:0,y:0},
          hipL: currentSkeleton.joints.LHip || {x:0,y:0},
          hipR: currentSkeleton.joints.RHip || {x:0,y:0},
          kneeL: currentSkeleton.joints.LKnee || {x:0,y:0},
          kneeR: currentSkeleton.joints.RKnee || {x:0,y:0},
          ankleL: currentSkeleton.joints.LAnkle || {x:0,y:0},
          ankleR: currentSkeleton.joints.RAnkle || {x:0,y:0}
      };
  }

  drawSkeleton(ctxInc, legacyJoints, wInc, hInc, incOptions);
}
...
// Удалены переменные MediaPipe и formatMediaPipeLandmarks
...
// Удалена старая логика classifyJump, updateEdgeChart, initMediaPipe и processVideoFrameLoop
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // MediaPipe инициализация удалена
  ...
  addChatMessage('coach', 'Hi! I am your AI Coach. Upload a video to start the OpenPose + Mistral analysis.');
});
...
function setupEventListeners() {
  ...
  const fileBtn = document.querySelector('.upload-btn');
  if (fileInput && fileBtn) {
    fileInput.addEventListener('change', (e) => {
      if (e.target.files.length > 0) {
        const file = e.target.files[0];
        fileBtn.querySelector("span").textContent = "Uploading & OpenPose...";
        ...
        const uploadTimer = setInterval(() => {
            secondsPassed++;
            const timerElem = document.getElementById('loading-timer-text');
            if (timerElem) {
                timerElem.innerHTML = 'Extracting 2D skeleton and running AI analysis... Time: <span style="color:#FFF;">' + secondsPassed + 's</span>';
            }
        }, 1000);
        ...
        .then(finalAnswer => {
            ...
            window.openPoseTimeline = finalAnswer.timeline || [];
            fileBtn.querySelector("span").textContent = "Upload Video";

            document.getElementById('hdr-base').textContent = (finalAnswer.baseScore||0).toFixed(2) + " pts";
        ...
        .catch(err => {
            ...
            fileBtn.querySelector("span").textContent = "Upload Error";
            if (diagCauseElem) diagCauseElem.innerHTML = '<div style="color: #EF4444; font-weight: bold;">OpenPose or Mistral API Server Error</div>';
        });
      }
    });
  }
...
function handleVideoTimeUpdate(e) {
  ...
  if (lastVideoTimeTracker > currentTime && currentTime < startTime + 1.0) {
      // Do not reset analysis state on loop so the data remains visible!
  }
...
function renderFrameUI() {
  drawOpenPoseOverlay(document.getElementById('video-canvas'), activeJump, currentFrame);
